...
describe('DELETE /friend/:_id', () => {
    ...
    beforeEach('Sign up users for test', async () => {
        ...
        await FriendService.acceptRequest(idUser2, idUser1);
    });
    ...
    it('Cannot remove friend with invalid id', async () => {
        const response = await request(app)
            .delete('/friend/' + 123)
            .set({ token: token2 });
        ...
        const user2 = await User.findById(idUser2);
        equal(user2.friends.length, 1);
    });

    it('Cannot remove friend with invalid token', async () => {
        const response = await request(app)
            .delete('/friend/' + idUser1)
            .set({ token: 'a.b.c' });
        equal(response.status, 400);
    ...
    it('Cannot remove friend for idUser3', async () => {
        const response = await request(app)
    ...
    it('Cannot remove friend for idUser3', async () => {
        await FriendService.addFriend(idUser3, idUser2);
        const response = await request(app)
            .delete('/friend/' + idUser3)
            .set({ token: token2 });
        equal(response.status, 404);
